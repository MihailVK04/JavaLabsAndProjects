...
public class RomanticErgenka implements Ergenka {
    ...
    public void reactToDate(DateEvent dateEvent) {
        int bonus;
        if ( dateEvent.getLocation().equals(favoriteDateLocation) ) {
            bonus = 5;
        }
        else if ( dateEvent.getDuration() < MIN_DATE_DURATION) {
            bonus = -3;
        }
        else if ( dateEvent.getDuration() > MAX_DATE_DURATION) {
            bonus = -2;
        }
        else {
            bonus = 0;
        ...
        rating = ( (romanceLevel * 7) / dateEvent.getTensionLevel() ) + humorRating + bonus;
    }
}
